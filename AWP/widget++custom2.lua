...
function WidgetsPlusPlusCustom.flushOverRide(self) --replace the flush thrust
    ...
    if lockedDetinationX ~= nil and lockedDetinationY ~= nil and lockedDetinationZ ~= nil then

        local cWP = DUConstruct.getWorldPosition()
        local cWPx, cWPy, cWPz = cWP[1], cWP[2], cWP[3]
        local lDx, lDy, lDz = lockedDetinationX - cWPx, lockedDetinationY - cWPy, lockedDetinationZ - cWPz
        ...
        upInput ~= 0 or 
        forwardInput ~= 0 then
            lockedDetinationX = nil
            lockedDetinationY = nil
            lockedDetinationZ = nil
    ...
    if params.Travel_Planner.Destination.value ~= nil and #params.Travel_Planner.Destination.value == 3 then
        local bf = function() return function()
                            if mouseWheel == 0 then
                                    ...
                                    lockedDetinationX = params.Travel_Planner.Destination.value[1]
                                    lockedDetinationY = params.Travel_Planner.Destination.value[2]
                                    lockedDetinationZ = params.Travel_Planner.Destination.value[3]
