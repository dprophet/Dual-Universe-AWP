...
function WidgetsPlusPlusCustom.SVG_Update(self)
    ...
    if delay >= 60 then
        ...
        local BBC = params.Menu_Settings.BUTTON_BORDER_COLOR.value
        local BC = params.Menu_Settings.BUTTON_COLOR.value
        local WC = params.Menu_Settings.WINDOW_COLOR.value --
        local AC = params.Menu_Settings.WIDGET_ANIM_COLOR.value
        local FC = params.Menu_Settings.WIDGET_FIXED_COLOR.value
        ...
        if params.Travel_Planner.Destination.value ~= nil and #params.Travel_Planner.Destination.value == 3 then
            interactablePOIS[entriesTotNum+1] = {center = params.Travel_Planner.Destination.value, name = {'Destination'}, radius = 1000, type = {'Destination'}}
        else
        ...
        for i, entry in ipairs(interactablePOIS) do
            ...
            local bf = function() return function()
                                    ...
                                    params.Travel_Planner.Destination.value = destPos
                                    windowsShow()
                                    end end
