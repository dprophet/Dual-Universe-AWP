...
local clamp, abs, sqrt, min = utils.clamp, math.abs, math.sqrt, math.min
MasterModeList = {
    "CRUISE",
    "TRAVEL",
    "PARKING",
...
--DUSystem.print("1aa")
    local cAVx, cAVy, cAVz = cAV[1], cAV[2], cAV[3]
    local xSpeedMPS = cAVx
    local ySpeedMPS = cAVy
    local zSpeedMPS = cAVz
    local xyzSpeedMPS = sqrt(cAVx^2+cAVy^2+cAVz^2)
    local xyzSpeedKPH = xyzSpeedMPS*3.6
...
--DUSystem.print("1a")
    ...
    local AxisSpeed = 0
    local tSpeed = clamp(speed, -self.maxSpeedKMPH+100, self.maxSpeedKMPH-100)
    if inspace == 0 and tSpeed ~= 0 then
        tSpeed = min(tSpeed,self.atmoMaxSpeed)
    end
